...
3.  Practice per-recruit and and equilibrium calculations for age-structured fish populations

4.  Practice equilibrium reference point calculation for age-structured fish populations

<br>
<br>

![](images/grim.png){.absolute bottom=3% right=0 width=30%}
...
$$
B_{\text{MSY}} = \frac{K}{2}, \quad U_{\text{MSY}} = \frac{r}{2}, \quad Y_{\text{MSY}} = \frac{rK}{4}
$$
...
## Enduring themes from the Golden Era of harvesting theory: maximizing yield 
...
## Enduring themes from the Golden Era of harvesting theory: risk-averse utility
...
- Biological reference points, which are those linked specifically to the mathematics of a model, are determinable by an analyst in an objective way
  - All others require value judgements, risk thresholds, or policy preferences, which by definition make them subjective 
- Feedback policies can maximize yield or risk-averse utility (or anything else)
- Equilibrium and simulation based reference points are much simpler to compute than feedback policies

:::

[Hilborn and Walters 1992; Walters and Martell 2004]{.footerRight}
...
- Sustainability assessments are complex, but best practices require the determination of one or more reference points
- Feedback policies difficult to compute but very powerful and provide much of the theoretical ba 


## Survivorship in the unfished condition
...
\begin{aligned}
  &1                               && \text{if } a = 1 \\
  &\mathrm{lo}_{a-1} \cdot e^{-M}  && \text{if } 2 \leq a < a_{\max} \\
  &\dfrac{\mathrm{lo}_{a_{\max}-1}}{1 - e^{-M}} && \text{if } a = a_{\max}
\end{aligned}
\right.
$$
...
- lo is survivorship at age vector in the unfished condition
...
$$
\mathrm{lo}_{a_{\max}} = \sum_{j=1}^\infty \mathrm{lo}_{a_{\max}-1} \cdot (e^{-M})^j
$$

This is a geometric series with:

$$
\sum_{j=1}^\infty r^j = \frac{r}{1 - r}, \quad \text{where } r = e^{-M}
$$

So:

$$
\mathrm{lo}_{a_{\max}} = \mathrm{lo}_{a_{\max}-1} \cdot \frac{e^{-M}}{1 - e^{-M}} = \frac{\mathrm{lo}_{a_{\max}-1}}{1 - e^{-M}}
$$
...
$$
\mathrm{sbro} = \sum_{a=1}^{a_{max}} \mathrm{lo}_a \cdot \mathrm{m}_a \cdot \mathrm{w}_a
$$

[for more information see Walters and Martell 2004 box 3.1]{.footerRight}
...
- DFO Precautionary Approach. 2009.  [https://www.dfo-mpo.gc.ca/reports-rapports/regs/sff-cpd/precaution-eng.htm](https://www.dfo-mpo.gc.ca/reports-rapports/regs/sff-cpd/precaution-eng.htm)
...
-   Restrepo and Powers 1999. Precautionary control rules in US fisheries management:
