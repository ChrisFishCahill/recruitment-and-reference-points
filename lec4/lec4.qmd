...
-   Feedback policies (also called harvest control rules) are agreed-upon, transparent, mathematical equations that adjust harvest in response to changes in stock size
    -   Intended to meet explicit management objectives (e.g., stable yield, rebuilding, consistent catches)\
...
-   This is surprising—because complex systems might seem to require complex strategies—but much of the work from the 1970s and 1980s showed that **simple feedback policies** often perform well or are even optimal (e.g., Walters 1986; Hilborn and Walters 1992).

-   This body of work forms a large part of the theoretical basis of the *de facto* precautionary rules used by e.g., NOAA, DFO, and others.
:::
...
-   Regardless of our individual thoughts on this objective, we can probably agree that this isn't your grandad's MSY and harvesting at rates above it is unwise
:::
...
```{R echo = T, eval = T}
get_upow <- function(X, H) {
  # X = guaranteed catch
  # H = high value of 50/50 gamble (other outcome is 0)
  log(0.5) / (log(X) - log(H))
}

# Example: Cahill is indifferent between 3 guaranteed and 
# a 50/50 gamble between 0 or 20 walleye
get_upow(X = 3, H = 20)
...
::: {style="font-size:26px"}
-   Let us ask pose the following thought-experiment
    ...
    -   What would you do differently if you knew what was going to happen?
:::
...
-   Recognize that the approach differs under:
    -   **Input control**: $U_t$ (or $F_t$) is implemented, and the resulting catch is implied.
    -   **Output control**: where $\text{TAC}_t$ is implemented and the resulting $U_t$ (or $F_t$) is implied from harvest over true biomass
...
- Explore how objectives (yield vs. HARA) changes feedback policy shape
- Does observation error change your answers? If so, how?
- Discuss value of information and the dynamics of uncertainty in policy design
